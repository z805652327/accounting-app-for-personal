...
'    var el = document.querySelector(".hero-amount"); if(el) el.textContent = (286420 + totalIncome - totalExpense).toFixed(2);',
'    var inc = document.querySelector(".flow-amount.income"); if(inc) inc.textContent = "¥" + totalIncome.toFixed(2);',
'    var exp = document.querySelector(".flow-amount.expense"); if(exp) exp.textContent = "¥" + totalExpense.toFixed(2);',
'    if (txns.length > 0) {',
'      var rows = txns.map(function(tx) {',
'        var isExp = ["expense","loan_repay","credit_card_spend"].includes(tx.txType);',
...
'      }).join("");',
'      var section = document.querySelector(".tx-section");',
'      if(section) section.innerHTML = "<div class=\"tx-header\"><span class=\"tx-header-title\">最近交易</span></div>" + rows;',
'    }',
...
html = html.replace('</body>', dbScript + '\n</body>');
